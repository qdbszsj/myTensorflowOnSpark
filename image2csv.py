...
def main(db_path, db_name, test_size, cpu_cores, tfrecords_bath_dir):
    ...
    print("data/wiki_crop/"+data_sets.values[10,1][0])
    image = mpimg.imread("data/wiki_crop/"+data_sets.values[10,1][0])
    print(image.shape)
    plt.imshow(image)
    plt.show()
    '''
    ...
    image = cv2.imread("data/wiki_crop/"+data_sets.values[15,1][0], cv2.IMREAD_COLOR)
    print(image.shape)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    rects = detector(gray, 2)
    ...
    data_sets = data_sets[data_sets.age <= 10]
